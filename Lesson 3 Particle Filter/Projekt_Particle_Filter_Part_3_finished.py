...
class robot:
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        local_robot = self
        x_set = random.gauss(motion[1], self.distance_noise)
        heading_set = random.gauss(motion[0], self.steering_noise)

        beta = (x_set / self.length) * tan(heading_set)
        if beta < 0.001:
            local_robot.x = self.x + x_set * cos(self.orientation)
            local_robot.y = self.y + x_set * sin(self.orientation)
            local_robot.orientation = (self.orientation + beta) % (2 * pi)
            ...
            local_robot.x = cx + sin(self.orientation + beta) * R
            local_robot.y = cy - cos(self.orientation + beta) * R
            local_robot.orientation = (self.orientation + beta) % (2 * pi)
